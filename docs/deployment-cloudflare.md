...
```text
https://<worker-domain>/atlas
https://<worker-domain>/atlas/config
```

不要使用 `/atlas*` 这类通配路径，它可能同时匹配 `/atlas-router` 订阅入口。Access policy 只允许自己的账号访问。然后在 Worker 的 Settings -> Variables and Secrets 中配置：
