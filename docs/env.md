...
`mitm` 是可选字段。通过 `/admin` 新增或复制用户时，Worker 会为该用户生成独立 CA，并保存在对应 profile 的 `mitm` 中。订阅输出会把 `caP12` 和 `caPassphrase` 渲染为 Surge `[MITM]` 的 `ca-p12` 和 `ca-passphrase`；`hostname` 为空时不会默认解密具体域名，可后续在管理页填写或用 Surge 模块追加。
